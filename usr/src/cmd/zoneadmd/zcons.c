...
devlinks:
	 ...
	 * everything set up yet due to heavy zone startup load. Wait for
	 * 1 sec. and retry a few times. Even if we can't set up the zone's
	...
	if (subfd == -1) {
		zerror(zlogp, B_TRUE,
		    "ERROR: could not open subsidiary side of zone"
		    " console for %s to acquire subsidiary handle", zone_name);
	}
